...
def _make_tracker_from_module(mod, cls_name):
    if mod is None:
        return None

    # old-style function API, e.g. cv2.legacy.TrackerKCF_create()
    fn = getattr(mod, f"{cls_name}_create", None)
    if callable(fn):
        return fn()

    # class API, e.g. cv2.legacy.TrackerKCF.create()
...
def create_face_tracker(name='KCF'):
    ...
    if key not in tracker_map:
        raise ValueError(f"Unknown tracker '{name}'. Available: {list(tracker_map.keys())}")

    cls_name = tracker_map[key]

    # try legacy first, then top-level cv2
    for mod in (getattr(cv2, "legacy", None), cv2):
        tracker = _make_tracker_from_module(mod, cls_name)
        if tracker is not None:
            return tracker

    raise RuntimeError(
        f"Tracker '{name}' is not available in this OpenCV build."
    )
